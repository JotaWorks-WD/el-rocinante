...
**`page.php` — terminates the unassigned-Page fallthrough.** Until now the parent shipped no `page.php`, so a Page left on Template = "Default" fell all the way through the hierarchy to `index.php`. That is the deploy footgun recorded in 360 Splendor's post-deploy checklist: a Home page set as the static front page but never assigned its template renders through the catch-all with body class `page-template-default`, `<main>` empty, and every stylesheet loading correctly the whole time. The new file does not fix a missing assignment — it gives the route a real, structured template instead of the catch-all. Its body mirrors `index.php` exactly (same `<main id="main-content" class="site-main">` wrapper, same loop), so the rendered output for an unassigned Page is unchanged; only which file produced it moves. A child that wants a designed default overrides it; a child shipping one template per real page never reaches it. New file at 1.0.0.

**The per-page CSS loader — `inc/pages/page-assets.php`, new file at 1.0.0.** Two functions. `roci_page_asset_slug()` resolves the current route to a bundle basename: on a singular route it reads the assigned template via `get_page_template_slug( get_queried_object_id() )` and derives the slug from the filename, keeping the `page-` prefix so the derived name matches the bundles already compiled in every repo in the network with **no renaming required**. `roci_enqueue_page_bundle()` then enqueues `dist/css/{slug}.css` on `wp_enqueue_scripts` **priority 20**, guarded on `file_exists()` **and** `filesize() > 0`, with handle `roci-page-{slug}` and a dependency on the child's base sheet.

**Why parent-side.** `get_stylesheet_directory()` resolves to the active **child** even when called from parent code, so a parent function can reach child assets; the inverse buys nothing, and a child-side loader would have to be written once per child and would be absent by default in the next one. The path is therefore built from `get_stylesheet_directory()` / `_uri()`, **not** the template variants — the same rule `inc/admin/brand-scheme.php:162-164` states for the inverse case: pick the directory function by where the *file* ships, never by where the *code* lives. A parent-dir lookup here would resolve to the parent and find nothing on exactly the sites that have a bundle.

Five details in the resolver are decisions rather than defaults, and each is commented in place:

- **`sanitize_file_name()` is security-critical.** `_wp_page_template` is post meta, editable through the REST API by anyone who can edit the page, and this function turns it into a filesystem path. Without sanitising, a crafted value containing `../` is a local file inclusion.
- **The front page is NOT excluded.** The common form of this pattern early-returns on `is_front_page()`, assuming the homepage has a dedicated bundle outside the convention. That does not hold here: this network routes its homepage through a named template rather than `front-page.php` precisely so it behaves like every other page. It resolves to `page-home` and loads like any other route. Restoring the exclusion would silently disable the homepage bundle on any child that ships one.
- **Fallbacks are route-named, not `post-{ID}`.** `page-single`, `page-404`, `page-search`, `page-archive` — matching the bundles already on disk. A per-ID bundle would key a stylesheet to a database ID that does not survive a content migration.
- **`is_home()` returns no bundle, explicitly.** The posts-page route is the noindexed deep-pagination archive, not a designed page; the indexed blog landing is a named template and resolves normally. Checked before `is_archive()` so the specific condition wins.
- **The `is_singular()` block deliberately has no terminal return.** It is a wrapper, not an early-return guard: a singular route with no assigned template must fall through to the route-named fallbacks. Adding `return false;` there — the "obvious" tidy-up — would make every branch below it dead code silently, with no error and no warning. Commented at the closing brace for exactly that reason.

**Cascade correctness** comes from the declared dependency, resolved through the new filter `roci_page_asset_dependency` and defaulting to `get_stylesheet() . '-style'`. Both current children name their base handle that way, so the default resolves; the filter exists so a child that names it differently corrects it in one line instead of silently losing its page styles. Priority 20 places the loader after every existing enqueue in the parent and both children (all at 10), which guarantees the dependency handle is registered when it resolves.

**`filesize() > 0` is not defensive padding.** The per-page SCSS entry points are stubs on most sites, so their compiled output is zero bytes while the file itself exists. Guarding on existence alone would request and serve every stub as an empty 200 — a real round trip and a render-blocking stylesheet containing nothing.

**Blast radius today: none.** No child file was touched and no reader signature changed. The loader adds exactly one guarded `wp_enqueue_style()` call, writes no state, runs no query, and dequeues nothing. On both live children every current route bails at one of the two guards, so nothing new is enqueued anywhere until the corresponding SCSS stubs are filled — a Build-repo change, not this one. 360 Splendor is the site this unlocks: its four page bundles are real, non-empty, correctly named, and have never reached a browser.

**D9 — `inc/metabox/metabox-readers.php` 1.1.0 → 1.1.1, docblock only.** The `@param $page` description claimed that for multi-word templates "hyphens are dropped and lowercased," and cited CLAUDE.md §12.6 as its authority. Both halves were wrong: the code performs a bare concatenation and strips nothing, and §12.6 states the opposite — hyphens are **kept**. The claim was harmless only because nobody had implemented it; it was precisely the kind of statement a future normalisation helper would be written against, and following it would have produced option names that read nothing, silently, for every multi-word page. The sibling `roci_get_setting_raw()` docblock had always described the parameter correctly. Corrected to state that hyphens are preserved, with `pages/page-tide-weather.php` → `tide-weather` → `{prefix}_page_tide-weather` as the worked example. No executable line in the file changed.

PHP only: no SCSS, no JavaScript, no `dist` rebuild and no Build-repo counterpart. MINOR: additive — one new template, one new include, two new `roci_` functions and one new filter, with no change to any existing signature and no front-end rendering change on any site.
